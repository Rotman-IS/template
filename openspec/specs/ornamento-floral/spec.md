# ornamento-floral Specification

## Purpose

Lo que aporta la acuarela floral al ornamento de esquina: la prop opcional que la recibe, la derivación de las cuatro orientaciones desde un único asset, la contención dentro de la sección, y el hecho de que su ausencia sea un fallo de compilación y no una degradación silenciosa.

El comportamiento base del componente `Ornamento` —su carácter decorativo, su exclusión del árbol de accesibilidad, su posicionamiento en la esquina y el marcador de respaldo— vive en el requisito `Primitivo Ornamento` de `invitacion-placeholders`.
